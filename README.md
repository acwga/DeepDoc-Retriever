...
## 🛠️ 技术栈
- **语言模型**：通义千问 qwen3-max（答案生成）、qwen2.5:7b（查询改写/摘要）
- **向量模型**：SentenceTransformers（bge-small-en-v1.5）
...
3. **意图识别**：LLM判断问题是否需要检索，避免不必要的RAG调用
4. **查询优化**：LLM改写问题，结合对话历史，提高与英文技术文档的匹配度
5. **历史摘要**：长对话自动摘要，保留关键上下文
6. **可解释性**：答案附带引用文档，便于验证
7. **性能优化**：使用MD5作为文档块唯一标识，支持增量索引，降低内存占用
8. **最优参数搜索**：通过网格搜索和精细搜索，寻找最优超参
